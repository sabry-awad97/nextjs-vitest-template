...
export function getHotkeyHandler(hotkeys: Hotkey[]) {
  const onKeyDown = (event: KeyboardEvent) => {
    ...
    const hotkeyPressed = hotkeys.find(([hotkey]) => {
      ...
      const hasModifier = modifiers ? !!event[(modifiers + 'Key') as keyof typeof event] : !isModifierPressed;
...
const useHotkeys = (hotkeys: Hotkey[], ignoreElements: ElementSelector) => {
  const shouldIgnoreElement = (element: HTMLElement) => ignoreElements.some(selector => element.matches(selector));

  const handleHotkeyEvent = useCallback((event: KeyboardEvent) => getHotkeyHandler(hotkeys)(event), [hotkeys]);
